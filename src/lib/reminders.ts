...
export function checkReminders(settings: AppSettings): ReminderCheck[] {
  ...
  if (settings.notificationsEnabled) {
    ...
    const lastWaterCheck = parseInt(localStorage.getItem("fitness_last_water_reminder") || "0", 10);
    const nowMs = now.getTime();
    if (nowMs - lastWaterCheck > interval * 60 * 60 * 1000) {
      checks.push({ type: "water", message: "Time to drink water! Stay hydrated." });
      localStorage.setItem("fitness_last_water_reminder", String(nowMs));
    }
  }

  if (settings.mealReminderEnabled) {
    if (hour === 8 || hour === 12 || hour === 19) {
      const lastMealCheck = localStorage.getItem("fitness_last_meal_reminder") || "";
      ...
      if (lastMealCheck !== `${todayKey}-${hour}`) {
        ...
        localStorage.setItem("fitness_last_meal_reminder", `${todayKey}-${hour}`);
      }
    }
  }
  ...
  if (settings.workoutReminderEnabled) {
    if (hour === 17 || hour === 18) {
      const lastWorkoutCheck = localStorage.getItem("fitness_last_workout_reminder") || "";
      const todayKey = now.toISOString().split("T")[0];
      if (lastWorkoutCheck !== todayKey) {
        checks.push({ type: "workout", message: "Time for your workout! Let's crush it." });
        localStorage.setItem("fitness_last_workout_reminder", todayKey);
      }
    }
  }
